...
func (s *Server) HandleNewChan(conn *utils.StatConn, sconn *ssh.ServerConn, nch ssh.NewChannel) {
	...
	if s.proxyMode {
		...
		if channelType == "session" {
			...
			go s.handleSessionRequests(conn, sconn, identityContext, ch, requests)
		} else {
	...
	case "direct-tcpip":
		...
		go s.handleDirectTCPIPRequest(conn, sconn, identityContext, ch, req)
	default:
		nch.Reject(ssh.UnknownChannelType, fmt.Sprintf("unknown channel type: %v", channelType))
	}
}
...
func (s *Server) handleDirectTCPIPRequest(statconn *utils.StatConn, sconn *ssh.ServerConn, identityContext srv.IdentityContext, ch ssh.Channel, req *sshutils.DirectTCPIPReq) {
...
func (s *Server) handleSessionRequests(statconn *utils.StatConn, sconn *ssh.ServerConn, identityContext srv.IdentityContext, ch ssh.Channel, in <-chan *ssh.Request) {
